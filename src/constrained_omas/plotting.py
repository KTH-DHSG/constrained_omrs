...
def save_plots(
    ...
    if all_prospective:
        ...
        ax.axhline(
            scenario.reconfiguration.rho_activation_threshold,
            linestyle="--",
        ...
        for edge in all_prospective:
            ...
            ax.plot(result.time, margin, label=rf"$s_{{{edge[0] + 1},{edge[1] + 1}}}$")
        ax.axhline(0.0, linestyle="--", label="relaxed boundary")
        _add_switch_markers(ax, result)
        ax.set(
            xlabel="time [s]",
            ylabel="margin [m]",
    ...
    if result.dynamics_model in ("quadrotor", "quadrotor_rotor"):
        ...
        fig, ax = plt.subplots(figsize=(9, 4.6))
        for i in range(n):
            tracking = np.linalg.norm(result.acceleration_tracking_error[:, i], axis=1)
            ax.plot(
                result.time,
        ...
        ax.set(
            xlabel="time [s]",
            ylabel=r"$\|u_i-a_i\|$ [m/s$^2$]",
            title="High-level acceleration realization error",
        )
        ax.grid(True, alpha=0.3)
        ax.legend(ncol=3)
    ...
    ax.grid(True, alpha=0.3)
    ax.legend()
    path = output_dir / "open_system_counts.png"
    fig.tight_layout()
    fig.savefig(path, dpi=180)
    plt.close(fig)
    paths.append(path)
